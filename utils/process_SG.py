...
def gen_train_data(dataset, split, time_step, nb_nodes, nhood, global_att, batch_size, view='', reverse='0', D=False, graph=False, norm=True, L3=True, enc_k=10):
	...
	if dataset == 'KS20':
		...
		#
		# # real + complex
		# # EigVal, EigVec = sp.linalg.eigs(L, k=10 + 1, which='SR', tol=1e-2)  # for 40 PEs
		# # EigVec = EigVec[:, EigVal.argsort()]  # increasing order
		# pos_enc_ori = EigVec[:,1:10+1]

		# print(pos_enc_dim.size())

		# test
		# embedding_lap_pos_enc = nn.Linear(10, 64)
		# embedding_h = nn.Embedding(3, 64)
		# h = np.random.random([3]).astype(np.float32)
		# pos_enc_dim = pos_enc_dim.astype(np.float32)
		# input = torch.LongTensor([1])
		# pos_enc_dim = torch.from_numpy(pos_enc_dim)
		# h = embedding_h(input)
		# h_lap_pos_enc = embedding_lap_pos_enc(pos_enc_dim)
		# h_new = h + h_lap_pos_enc
		# print(h_new.size(), h.size(), h_lap_pos_enc.size())
		# exit()
	...
	# #idx = EigVal.argsort()
	# #EigVal, EigVec = EigVal[idx], np.real(EigVec[:, idx])
	# EigVal, EigVec = EigVal, np.real(EigVec)
	# # print(EigVal.shape, EigVec.shape)
	# # exit()
	# pos_enc_ori =  L #EigVec#[:, 1:k + 1]
	...
	idx = EigVal.argsort()
	EigVal, EigVec = EigVal[idx], np.real(EigVec[:, idx])
	concat = np.concatenate((np.expand_dims(L, axis=0), np.expand_dims(EigVec, axis=0)),axis=0)
	pos_enc_ori = concat #EigVec#[:, 1:k + 1]
		...
		if graph:
			...
			if not L3:
				return X_train_J, 0, 0, 0, 0, y_train, X_test_J, 0, 0, 0, 0, y_test, \
					   adj_joint, biases_joint, 0, 0, 0, 0, 0, 0, 0, 0, nb_classes
			else:
