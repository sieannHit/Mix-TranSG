...
tf.app.flags.DEFINE_string('St_mask_num', '10',
						   "")  # number of random masks (from 1 to (J-1)) in structure prompted reconstruction #10
...
# 		  '_lambda_' + FLAGS.seq_lambda + '_beta_' + FLAGS.prompt_lambda  + '_alpha_' + FLAGS.GPC_lambda#ZZA

change += '_f_' + FLAGS.length + '_layers_' + FLAGS.L_transformer + '_heads_' + FLAGS.n_heads + \
		  '_alpha_' + FLAGS.seq_lambda + '_beta_' + FLAGS.prompt_lambda  + '_lambda_' + FLAGS.GPC_lambda#ZZA
try:
	os.mkdir(pre_dir)
except:
	pass
...
if FLAGS.mode == 'Train':
	...
	with loaded_graph.as_default():
		with tf.name_scope('Input'):
			...
			L_eig = tf.placeholder(dtype=tf.float32, shape=(2, joint_num, joint_num))
		...
		with tf.name_scope("TranSG"), tf.variable_scope("TranSG", reuse=tf.AUTO_REUSE):
			...
			edge = tf.squeeze(L_eig[0]) #
			edge = tf.reshape(edge, [1, 1, joint_num, joint_num,1])
			edge = tf.layers.dense(edge, int(FLAGS.H), activation=tf.nn.relu)
			edge = tf.layers.dense(edge, int(FLAGS.H), activation=None)
			#edge = tf.tile(edge, [batch_size, time_step, 1, 1, 1])
			# pos_enc_S = tf.Variable(initial_value=tf.zeros([1, 1, joint_num, int(FLAGS.H)]))
			pos_enc_S = tf.squeeze(L_eig[1,:,1:int(FLAGS.enc_k)+1])
			pos_enc_S = tf.reshape(pos_enc_S, [1, 1, joint_num, int(k)])#int(FLAGS.enc_k)])
			pos_enc_S = tf.layers.dense(pos_enc_S, int(FLAGS.H), activation=tf.nn.relu)
			pos_enc_S = tf.layers.dense(pos_enc_S, int(FLAGS.H), activation=None)
			#pos_enc_T = tf.Variable(initial_value=tf.zeros([1, 1, time_step, int(FLAGS.H)]))
			pos_enc_T = tf.reshape(tf.linspace(1.0, tf.to_float(time_step), time_step), [1, 1, time_step, 1])
			pos_enc_T = tf.layers.dense(pos_enc_T, int(FLAGS.H), activation=tf.nn.relu)
			pos_enc_T = tf.layers.dense(pos_enc_T, int(FLAGS.H), activation=None)
			# sess = tf.InteractiveSession()
			# print(pos_enc_T.eval())
			if FLAGS.pos_enc == '1':
				h_S = seq_ftr + pos_enc_S
				h_T = tf.transpose(seq_ftr, perm=[0, 2, 1, 3]) + pos_enc_T
			...
			#(2)
			#seq_ftr = tf.transpose(h_T, perm=[0, 2, 1, 3]) + pos_enc_S

			# (3)
			# h_S_temp = tf.expand_dims(h_S, -1)
			# h_T_temp = tf.expand_dims(tf.transpose(h_T, perm=[0, 2, 1, 3]), -1)
			# seq_ftr = tf.concat([h_S_temp, h_T_temp], axis=-1)
			# seq_ftr = tf.layers.dense(seq_ftr, 4, activation=None)
			# seq_ftr = tf.layers.dense(seq_ftr, 1, activation=None)
			# seq_ftr = tf.squeeze(seq_ftr)

			H = int(FLAGS.H)
			W_head = lambda: tf.Variable(tf.random_normal([H, H // int(FLAGS.n_heads)]))
			W_m_ffn = lambda: tf.Variable(tf.random_normal([H // int(FLAGS.n_heads), H // int(FLAGS.n_heads)]))
			...
			for l in range(int(FLAGS.L_transformer)):
				for i in range(int(FLAGS.n_heads)):
					...
					W_E = tf.Variable(initial_value=W_head)
					#W_E_ffn = tf.Variable(initial_value=W_m_ffn)
					Q_h = tf.matmul(h, W_Q)
					K_h = tf.matmul(h, W_K)
					K_h = tf.transpose(K_h, perm=[0, 1, 3, 2])
					E_h = tf.matmul(edge, W_E)

					## attention map
					att_scores = tf.clip_by_value(tf.matmul(Q_h, K_h) / np.sqrt(H // int(FLAGS.n_heads)),  -5, 5)
					# edge
					att_scores = tf.expand_dims(att_scores, -1) + E_h
					#E_h_atten = tf.reshape(tf.reduce_sum(tf.reduce_sum(att_scores, axis=0), axis=0), [1, 1, joint_num, joint_num, -1])
					#E_h_atten = tf.matmul(E_h_atten, W_E_ffn)

					V_h = tf.matmul(h, W_V)

					# #(1)
					# att_scores = tf.squeeze(tf.reduce_sum(att_scores,axis=-1))
					# # numerical stability to clamp [-5, 5]
					# att_scores = tf.nn.softmax(tf.clip_by_value(att_scores, -5, 5))
					# aggr_features = tf.matmul(att_scores, V_h)
					...
					if i == 0:
						...
						#concat_edge = E_h_atten
					else:
						concat_features = tf.concat([concat_features, aggr_features], axis=-1)
						#concat_edge = tf.concat([concat_edge, E_h_atten], axis=-1)
				h = concat_features
				# if l==0:
				# 	edge = tf.tile(edge, [batch_size, time_step, 1, 1, 1])
				#concat_edge = tf.layers.dense(concat_edge, H, activation=None)

				# #(1)
				# edge = tf.concat([edge, concat_edge], axis=-1)
				# edge = tf.layers.dense(edge, 2*H, activation=tf.nn.relu)
				# edge = tf.layers.dense(edge, H, activation=None)

				# #(2)
				# edge = tf.concat([tf.expand_dims(edge, -1), tf.expand_dims(concat_edge, -1)], axis=-1)
				# #edge = tf.layers.dense(edge, 4, activation=tf.nn.relu)
				# edge = tf.squeeze(tf.layers.dense(edge, 1, activation=None), axis=-1)

				#(3)
				#concat_edge = tf.squeeze(tf.layers.dense(tf.expand_dims(concat_edge, -1), 1, activation=None), axis=-1)
				#edge = edge + concat_edge
			...
			for l in range(int(FLAGS.L_transformer)):
				for i in range(int(FLAGS.n_heads)):
					W_Q_t = tf.Variable(initial_value=W_head)
					W_K_t = tf.Variable(initial_value=W_head)
					W_V_t = tf.Variable(initial_value=W_head)
					Q_h_t = tf.matmul(h, W_Q_t)
					K_h_t = tf.matmul(h, W_K_t)
					K_h_t = tf.transpose(K_h_t, perm=[0, 1, 3, 2])
					# numerical stability to clamp [-5, 5]
					att_scores = tf.nn.softmax(
						tf.clip_by_value(tf.matmul(Q_h_t, K_h_t) / np.sqrt(H // int(FLAGS.n_heads)), -5, 5))
					# [batch_size, joint_num, time_step, time_step]
					V_h_t = tf.matmul(h, W_V_t)
					aggr_features = tf.matmul(att_scores, V_h_t)
			...
			h_T = tf.transpose(h_T, perm=[0, 2, 1, 3])
			h_T = tf.expand_dims(h_T, -1)
			h = tf.concat([h_S, h_T], axis=-1)
			h = tf.layers.dense(h, 4, activation=tf.nn.relu)
			h = tf.layers.dense(h, 1, activation=None)
			h = tf.squeeze(h)

			# (2)
			#h = (h_S + tf.transpose(h_T, perm=[0, 2, 1, 3])) * 0.5

			# #(3)
			# W_mix = tf.reshape(tf.Variable(initial_value=W_2_to_1), [1, 1, 1, H, 2])
			# h_S = tf.expand_dims(h_S, -1)
			# h_T = tf.transpose(h_T, perm=[0, 2, 1, 3])
			# h_T = tf.expand_dims(h_T, -1)
			# h = tf.concat([h_S, h_T], axis=-1)
			# h = h * W_mix
			# h = tf.squeeze(tf.reduce_mean(h, axis=-1))
			...
			if FLAGS.S_prompt != '0':
				...
				h_S_m = tf.squeeze(h_S)#zza
				mask_G = tf.boolean_mask(h_S_m, tf.reshape(node_mask, [-1]), axis=-2)#zza
			...
			if FLAGS.T_prompt != '0':
				...
				h_T_m = tf.squeeze(h_T)#zza
				part_T_enc_1 = tf.boolean_mask(h_T_m, tf.reshape(seq_mask, [-1]), axis=1)#zza
	...
	with tf.Session(graph=loaded_graph, config=config) as sess:
		...
		seq_ftr = loaded_graph.get_tensor_by_name("TranSG/TranSG/Reshape_24:0")#("TranSG/TranSG/Reshape_38:0")
